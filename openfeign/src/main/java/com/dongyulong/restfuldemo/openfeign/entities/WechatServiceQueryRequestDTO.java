package com.dongyulong.restfuldemo.openfeign.entities;
...
import com.dongyulong.restfuldemo.openfeign.config.Required;
import lombok.Data;
...
public class WechatServiceQueryRequestDTO {
     ...
     * 微信的订单号，建议优先使用
     */
    @Required(must = false, max = 32, params = "outTradeNo")
     ...
     */
    @Required(must = false, max = 32, params = "transactionId")
    ...
    public void sign(String key) {
        ...
        BeanUtil.beanToMap(map, targetMap, CopyOptions.create().ignoreNullValue().setIgnoreProperties("sign"));
        String collect = targetMap.entrySet().stream().map(this::joinString).collect(Collectors.joining("&"));
        sign = DigestUtils.md5Hex(String.format("%s&key=%s", collect, key).getBytes(StandardCharsets.UTF_8)).toUpperCase();
    }
